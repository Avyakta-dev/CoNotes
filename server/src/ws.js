...
function setupWS(server) {
  ...
  try {
    ...
    console.log('[ws] bad URL:', request.url);
    socket.destroy();
    return;
  }
  ...
  console.log('[ws] upgrade request for doc:', documentId, 'token present:', !!token);
  ...
  console.log('[ws] token verified for user:', user.email);

  const doc = db.prepare('SELECT owner_id FROM documents WHERE id = ?').get(documentId);
  if (!doc) {
    console.log('[ws] no document found with id:', documentId);
  ...
  if (!hasAccess) {
    console.log('[ws] user has no access to this document');
    return socket.destroy();
  }

  console.log('[ws] access granted, upgrading connection');
